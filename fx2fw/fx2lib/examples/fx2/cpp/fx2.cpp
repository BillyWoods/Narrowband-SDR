...
#ifdef DEBUG_DEVICE_OPEN
    printf("Looking for device %04x:%04x instance %d\n", vid, pid, idx);
#endif
    libusb_device **list;
...
#ifdef DEBUG_DEVICE_OPEN
          printf("Looking at device %04x:%04x\n", dscr.idVendor, dscr.idProduct);
#endif
          if ( dscr.idVendor == vid && dscr.idProduct == pid ) {
             if ( idx == cur_idx++ ) {
                int rv = libusb_open( list[i], &dev_handle);
                if (!rv) {
                    rv = libusb_set_auto_detach_kernel_driver(dev_handle, 1);
#ifdef DEBUG_DEVICE_OPEN
                    printf("libusb_set_auto_detach_kernel_driver: ");
                    switch (rv) {
                      case 0:                          printf("SUCCESS\n"); break;
                      case LIBUSB_ERROR_NOT_SUPPORTED: printf("LIBUSB_ERROR_NOT_SUPPORTED\n"); break;
                      default:                         printf("Returned: %d\n", rv);
                    }
#endif
                    ...
                    if (!rv) {                     interface=0;
                     ...
                     if (rv) {
#ifdef DEBUG_DEVICE_OPEN
                        printf("Could not set interface alt setting! Returned %d\n", rv);
#endif
                    ...
                    printf ( "Unable to open device idx: %d, ret: ", idx );
                    switch (rv) {
                      case 0:                      printf("SUCCESS\n"); break;
                      case LIBUSB_ERROR_NO_MEM:    printf("LIBUSB_ERROR_NO_MEM\n"); break;
                      case LIBUSB_ERROR_ACCESS:    printf("LIBUSB_ERROR_ACCESS\n"); break;
                      case LIBUSB_ERROR_NO_DEVICE: printf("LIBUSB_ERROR_NO_DEVICE\n"); break;
                      default:                     printf("%d\n", rv);
                    }
                }
